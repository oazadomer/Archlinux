...
if [[ $KERNEL == "1" ]] then
    pacstrap -K /mnt base base-devel linux linux-firmware linux-headers gvim grub efibootmgr grub-btrfs btrfs-progs inotify-tools git wget curl reflector rsync networkmanager networkmanager-openvpn wpa_supplicant usb_modeswitch nss-mdns modemmanger iwd ethtool dnsutils dnsmasq dhclient wireless-regdb wireless_tools smarmontools mtools net-tools dosfstools efitools nfs-utils nilfs-utils ntfs-3g ntp openssh cronie bash-completion bash-language-server pacman-contrib pkgfile rebuild-detector 
else
    pacstrap -K /mnt base base-devel linux-lts linux-firmware linux-lts-headers gvim grub efibootmgr grub-btrfs btrfs-progs inotify-tools git wget curl reflector rsync networkmanager networkmanager-openvpn wpa_supplicant usb_modeswitch nss-mdns modemmanger iwd ethtool dnsutils dnsmasq dhclient wireless-regdb wireless_tools smarmontools mtools net-tools dosfstools efitools nfs-utils nilfs-utils ntfs-3g ntp openssh cronie bash-completion bash-language-server pacman-contrib pkgfile rebuild-detector
fi
...
if [[ $DESKTOP == "1" ]] then
    ...
    pacman -Sy cutefish fishui cutefish-qt-plugins libcutefish ark polkit-kde-agent kwin kitty kitty-shell-integration kitty-terminfo starship ptop yazi sddm xdg-utils xdg-user-dirs xdg-user-dirs-gtk touchegg exfatprogs f2fs-tools traceroute gufw xdg-desktop-portal-gtk gnome-online-accounts mailspring-bin transmission-gtk simple-scan kdenlive audacity vlc mplayer video-downloader shutter-encoder-bin cheese flameshot gthumb gimp xournalpp pencil protonvpn-gui bookworm gparted gvfs-afc gvfs-goa gvfs-google gvfs-mtp gvfs-gphoto2 gvfs-nfs xz unrar unzip lzop gdb mtpfs php nodejs npm yarn ripgrep python-pip pyenv android-tools vala tk filezilla brave-bin downgrade debtap dpkg vscodium postman-bin xclip python-xlib gtk-engine-murrine orchis-theme candy-icons-git papirus-folders-nordic papirus-folders dracula-gtk-theme-git catppuccin-gtk-theme-mocha bibata-cursor-theme kvantum xampp docker --noconfirm --needed
    pacman -S timeshift timeshift-autosnap ventoy-bin crow-translate appimagelauncher megasync-bin fastfetch bleachbit --noconfirm --needed
    pacman -S zsh zsh-autosuggestions zsh-completions zsh-syntax-highlighting zsh-history-substring-search --noconfirm --needed
