...
reflector --latest 20 --sort rate --fastest 10 --save /etc/pacman.d/mirrorlist
pacman -Sy
...
if [[ $KERNEL == "1" ]] then
    pacstrap -K /mnt base base-devel linux linux-firmware linux-headers zsh fish gvim grub efibootmgr inotify-tools git python rust gcc make cmake less wget curl libaio reflector rsync networkmanager usb_modeswitch wireless_tools smartmontools mtools net-tools dosfstools efitools nfs-utils nilfs-utils exfatprogs ntfs-3g ntp openssh cronie bash-completion pacman-contrib pkgfile rebuild-detector mousetweaks usbutils os-prober                                      
else
    pacstrap -K /mnt base base-devel linux-lts linux-firmware linux-lts-headers zsh fish gvim grub efibootmgr inotify-tools git python rust gcc make cmake less wget curl libaio reflector rsync networkmanager usb_modeswitch wireless_tools smartmontools mtools net-tools dosfstools efitools nfs-utils nilfs-utils exfatprogs ntfs-3g ntp openssh cronie bash-completion pacman-contrib pkgfile rebuild-detector mousetweaks usbutils os-prober                                                
fi
...
# pamac install thorium-browser-bin megasync-bin crow-translate papirus-folders-nordic local-by-flywheel-bin --no-confirm 
...
# pamac install thorium-browser-bin megasync-bin crow-translate papirus-folders-nordic local-by-flywheel-bin --no-confirm
    ...
    export TERMINAL="kitty"
      ...
      pacman -S fastfetch-git debtap yay xdg-terminal-exec-git ollama proton-vpn-gtk-app libappindicator-gtk3 gnome-shell-extension-appindicator papirus-folders ventoy-bin appimagelauncher telegram-desktop zsh-theme-powerlevel10k-git --noconfirm --needed
      pacman -S ttf-cascadia-code-nerd ttf-cascadia-mono-nerd ttf-dejavu-nerd ttf-firacode-nerd ttf-hack-nerd ttf-ubuntu-font-family noto-fonts noto-fonts-emoji ibus-typing-booster ttf-dejavu ttf-hanazono ttf-ms-fonts ttf-font-awesome awesome-terminal-fonts powerline-fonts --noconfirm --needed
# pamac install thorium-browser-bin megasync-bin crow-translate mailspring-bin papirus-folders-nordic local-by-flywheel-bin --no-confirm
