...
if [[ $KERNEL == "1" ]]; then
    retry_command pacstrap /mnt base base-devel linux linux-firmware linux-headers bash-completion neovim git curl perl make cmake wget gcc gawk reflector rsync networkmanager mtools dosfstools ntfs-3g cronie acpid touchegg             

elif [[ $KERNEL == "2" ]]; then
    retry_command pacstrap /mnt base base-devel linux-lts linux-firmware linux-lts-headers bash-completion neovim git curl perl make cmake wget gcc gawk reflector rsync networkmanager mtools dosfstools ntfs-3g cronie acpid touchegg                                  
fi
...
if [[ $DESKTOP == "1" ]]; then
      ...
      retry_command pacman -S gnome-shell gnome-control-center gnome-menus ghostty xdg-terminal-exec-git superfile starship gnome-bluetooth gnome-themes-extra gnome-keyring power-profiles-daemon gnome-backgrounds gnome-tweaks gnome-online-accounts --noconfirm --needed
      retry_command pacman -S topgrade ulauncher vscodium catppuccin-cursors-mocha transmission-gtk gnome-screenshot gnome-calculator gnome-calendar simple-scan shotwell snapshot gparted gnome-browser-connector btop file-roller gdm xdg-utils xdg-user-dirs-gtk f2fs-tools traceroute gufw xdg-desktop-portal-gtk xdg-desktop-portal-gnome gvfs-afc gvfs-goa gvfs-google gvfs-mtp gvfs-gphoto2 gvfs-nfs 7zip xz unrar unzip lzop gdb mtpfs nodejs-lts-jod npm yarn ripgrep python-pip pyenv android-tools vala tk brave-origin-bin dpkg yay --noconfirm --needed
      ...
      retry_command pacman -S plasma-desktop dolphin dolphin-plugins ark plasma-nm plasma-pa kdeplasma-addons kde-gtk-config powerdevil bluedevil kscreen btop sddm sddm-kcm xdg-utils xdg-user-dirs-gtk breeze-gtk pamac-tray-icon-plasma xdg-desktop-portal-gtk xdg-desktop-portal-kde kitty kitty-shell-integration kitty-terminfo xdg-terminal-exec-git superfile starship f2fs-tools traceroute gufw --noconfirm --needed
      retry_command pacman -S topgrade vscodium catppuccin-cursors-mocha qalculate-gtk merkuro skanlite qbittorrent ffmpegthumbs kamoso flameshot gthumb gparted gvfs-afc gvfs-goa gvfs-google gvfs-mtp gvfs-gphoto2 gvfs-nfs 7zip xz unrar unzip lzop gdb mtpfs nodejs-lts-jod npm yarn python-pip pyenv android-tools vala tk brave-origin-bin dpkg kvantum kvantum-qt5 yay --noconfirm --needed
      ...
      retry_command pacman -S hyprland hyprpolkitagent hypridle waybar swaync swaybg swaylock-effects wlogout wireplumber udisks2 nwg-look qt5ct grim slurp swappy wl-clipboard cliphist wofi xdg-desktop-portal-hyprland xdg-desktop-portal-gtk power-profiles-daemon blueman gnome-keyring kitty kitty-shell-integration kitty-terminfo xdg-terminal-exec-git nautilus sushi pavucontrol file-roller superfile starship btop sddm f2fs-tools gufw traceroute --noconfirm --needed
      retry_command pacman -S topgrade vscodium catppuccin-cursors-mocha brave-origin-bin snapshot gnome-calendar gnome-contacts transmission-gtk qalculate-gtk nodejs-lts npm yarn ripgrep python-pip pyenv android-tools vala tk 7zip xz unrar unzip lzop gdb mtpfs dpkg adwaita-qt6 adwaita-qt5 yay gvfs-afc gvfs-goa gvfs-google gvfs-mtp gvfs-gphoto2 gvfs-nfs xsane gparted imv --noconfirm --needed
#      retry_command pamac install hyprmod hyprpicker --no-confirm --needed
      ...
      retry_command pacman -S wayland wayland-utils wayland-protocols qt6-wayland qt5-wayland glfw-wayland xorg-xwayland xorg-xlsclients libxkbcommon --noconfirm --needed
      retry_command pacman -S niri polkit-gnome swayidle waybar swaync swaybg swaylock-effects wlogout wireplumber udisks2 nwg-look qt5ct grim slurp swappy wl-clipboard cliphist wofi xdg-desktop-portal-wlr xdg-desktop-portal-gtk power-profiles-daemon blueman gnome-keyring kitty kitty-shell-integration kitty-terminfo xdg-terminal-exec-git nautilus sushi pavucontrol file-roller superfile starship btop sddm f2fs-tools gufw traceroute qt5-wayland qt6-wayland --noconfirm --needed
      retry_command pacman -S topgrade vscodium catppuccin-cursors-mocha brave-origin-bin snapshot gnome-calendar gnome-contacts transmission-gtk qalculate-gtk nodejs-lts npm yarn ripgrep python-pip pyenv android-tools vala tk 7zip xz unrar unzip lzop gdb mtpfs dpkg adwaita-qt6 adwaita-qt5 yay gvfs-afc gvfs-goa gvfs-google gvfs-mtp gvfs-gphoto2 gvfs-nfs xsane gparted imv --noconfirm --needed
...
if [[ $PROGRAMS == "y" ]]; then
    retry_command pacman -S gimp audacity audacious mplayer mpd rmpc vlc shutter-encoder-bin --noconfirm -needed
