...
if [[ $KERNEL == "1" ]] then
    pacstrap -K /mnt base base-devel linux linux-firmware linux-headers gvim zed grub efibootmgr grub-btrfs btrfs-progs inotify-tools git wget curl reflector rsync networkmanager wireless_tools iwd smarmontools mtools net-tools dosfstools openssh cronie bash-completion bash-language-server xf86-input-libinput libinput touchegg
else
    pacstrap -K /mnt base base-devel linux-lts linux-firmware linux-lts-headers gvim zed grub efibootmgr grub-btrfs btrfs-progs inotify-tools git wget curl reflector rsync networkmanager wireless_tools iwd smarmontools mtools net-tools dosfstools openssh cronie bash-completion bash-language-server xf86-input-libinput libinput touchegg
fi
